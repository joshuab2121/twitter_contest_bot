...
ratelimit_search = [999, 999, 100]  # [limit,remaining,percent]
...
def CheckError(r):
    r = r.json()
    if 'errors' in r:
        LogAndPrint("We got an error message: " +
...
def CheckRateLimit():
    ...
    if ratelimit[2] < min_ratelimit:
        print("Ratelimit too low -> Cooldown (" +
              str(ratelimit[2]) + "%)")
        time.sleep(200)
        ...
        for res_family in r['resources']:
            for res in r['resources'][res_family]:
                ...
                if percent < 5.0:
                    ...
                    LogAndPrint(
                        res_family + " -> " + res + ": " + str(percent) + "  !!! <30% alert !!!")
                elif percent < 70.0:
                    print(res_family + " -> " +
                          res + ": " + str(percent))
...
def UpdateQueue():
    ...
    if len(post_list) > 0:
        ...
        if not ratelimit[2] < min_ratelimit_retweet:
            ...
            post = post_list[0]
            LogAndPrint(
                "Retweeting: " + str(post['id']) + " " + str(post['text'].encode('utf8')))
            ...
            print("Ratelimit at " +
                  str(ratelimit[2]) + "% -> pausing retweets")


# Check if a post requires you to follow the user.
# Be careful with this function! Twitter may write
# ban your application for following too aggressively
def CheckForFollowRequest(item):
    text = item['text']
    if any(x in text.lower() for x in follow_keywords):
        try:
            r = api.request(
                'friendships/create', {'screen_name': item['retweeted_status']
                                       ['user']['screen_name']})
            CheckError(r)
            LogAndPrint(
...
# Be careful with this function! Twitter may write ban
# your application for favoriting too aggressively
def CheckForFavoriteRequest(item):
    text = item['text']

    if any(x in text.lower() for x in fav_keywords):
        try:
            r = api.request('favorites/create',
                            {'id': item['retweeted_status']['id']})
            CheckError(r)
            LogAndPrint("Favorite: " +
                        str(item['retweeted_status']['id']))
        except KeyError:
...
def ScanForContests():
    ...
    if not ratelimit_search[2] < min_ratelimit_search:
        ...
        for search_query in search_queries:
            ...
            try:
                r = api.request(
                    'search/tweets', {'q': search_query, 'result_type': "recent", 'count': 100})
                CheckError(r)
                c = 0
                                ...
                                if item['retweet_count'] > 50:
                                    ...
                                    if is_retweet:
                                        print(id + " - " + screen_name + " retweeting " +
                                              original_id + " - " + original_screen_name + ": ")
                                        ignore_list.append(
                                            original_id)
                                        f_ign.write(
                                            original_id + "\n")
        ...
        print("Search skipped! Queue: " + str(len(post_list)) +
              " Ratelimit: " +
              str(ratelimit_search[1]) +
              "/" +
              str(ratelimit_search[0]) +
              " (" +
              str(ratelimit_search[2]) +
              "%)")
